function skuGenerator(){ //take titles from column in sheet, output titles & skus in another sheet if they don't already exist
    ...
    for (var i=0;i<ar1.length;i++) { //loop through new titles sheet, create new SKUs for any that don't already exist
    ...
    function skuFormatter (hashedTitle){ //take a hashed title, format it and then return it
    ...
    function arrayFromSheet(sheetName,row,col) { //create an array from a sheet. define sheet name as string, start row & column as int
      ...
      for(n=row;n<values.length;++n){ //start at row
    ...
    function GetMD5Hash(value) { //input a text value to hash and return the hashed value as text
